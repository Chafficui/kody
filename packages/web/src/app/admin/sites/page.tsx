...
export default function AdminSitesPage() {
  ...
  async function handleDelete(siteId: string) {
    if (!window.confirm(`Delete site "${siteId}"? This cannot be undone.`)) {
      return;
    }
    try {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Manage your Kody site configurations.
          </p>
        </div>
        <Link
          href="/admin/sites/new"
          className="rounded-lg bg-primary px-4 py-2 text-sm font-semibold text-white shadow-md shadow-primary/25 transition-colors hover:bg-primary-dark"
        >
          Create New Site
        </Link>
      </div>
      ...
      {sites.length === 0 ? (
        <div className="rounded-xl border border-border bg-background p-12 text-center">
          ...
          <Link
            href="/admin/sites/new"
            className="mt-6 inline-block rounded-lg bg-primary px-5 py-2.5 text-sm font-semibold text-white shadow-md shadow-primary/25 transition-colors hover:bg-primary-dark"
          >
            Create New Site
          </Link>
        </div>
